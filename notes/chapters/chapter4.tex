...
\dfn{A random \emph{experiment} is any activity in which there are at least two possible
...
\dfn{The \emph{sample space} of an experiment is the set of all possible outcomes, denoted
by \emph{S} or $\Omega$.}
...
\dfn{The \emph{complement} of an event $A$ contains every outcome in the sample space that is
not in $A$, denoted by $A'$.}
...
\dfn{The \IT{complement rule} says that for any event $A$, $P(A')=1-P(A)$} \\
\dfn{The \IT{general additional rule } says that $P(A\cup B)= P(A)+P(B)-P(A\cap B)$} \\
...
\end{equation}
For more than two variables, suppose the sample space is partitioned into $k$ disjoint events, \llist{A}{1}{2}{k}, none of which have a probability of 0, such that
\begin{equation}
    \sum^k_{i=1}P(A_i)=1
\end{equation}
