...
\dfn{Random Experiment}{Any activity in which there are at least two possible
outcomes and the result of the activity can not be predicted with absolute certainty.}

\nt{}{By this definition, all experiments are random.}

\dfn{outcome}{the result of an experiment.}

\dfn{Trial}{Each time the experiment is done.}
\subsection{Tree Diagrams}  % 4.1.2
\nt{}{skip}
\subsection{Sample Spaces}  % 4.1.3
\dfn{sample space}{the set of all possible outcomes of an experiment, denoted
by \emph{S} or $\Omega$.}
\subsection{Events}  % 4.1.4
\dfn{An \emph{event} is any collection of outcomes from an experiment.}

\ex{The sample space is one possible event.}
...
\dfn{complement}{every outcome of an event $A$ in the sample space that is not in $A$, denoted by $A'$.}
...
\dfn{complement rule}{for any event $A$, $P(A')=1-P(A)$}
\dfn{general additional rule}{$P(A\cup B)= P(A)+P(B)-P(A\cap B)$}
...
\dfn{Independence}{Two events are \emph{independent} if knowing the outcome of one does not affect the outcome of the other. Mathematically, we write}
\begin{align}
    P(A\vert B)&=P(A) \\
    P(B\vert A)&=P(B) \\
...
\dfn{Disjoint}{Two events are \emph{disjoint} if they can not possibly occur at the same time.}
\dfn{Independence}{Two events are \emph{independent} if the outcome of one does not impact the other.}
\begin{enumerate}
